...
def main():
    ...
    ############################################################################
        ...
        for register_name in REGISTER_MAP:
            if register_name not in [
                ...
                reset_value = REGISTER_MAP[register_name][
                    'RESET_VALUE'
                ].to_bytes(
    ...
    # The `config` command:
        ...
        if args.rx_dr_irq:  # TODO: Change to the new simplified version.
            ...
            if args.rx_dr_irq == 'enable':
                ...
                nrf24l01.w_register(
                    'CONFIG', new_register_value.to_bytes(1, 'big')
                )
        ...
        if args.rx_addr_p0:
            if args.rx_addr_p0 is True:
                ...
                if len(int(args.rx_addr_p0, 16).to_bytes(5, 'big')) == 5:
                    ...
                    nrf24l01.w_register(
                        'RX_ADDR_P0', rx_addr_p0.to_bytes(5, 'big')
                    )
                else:
        ...
        if args.rx_addr_p1:
            if args.rx_addr_p1 is True:
                ...
                if len(int(args.rx_addr_p1, 16).to_bytes(5, 'big')) == 5:
                    ...
                    nrf24l01.w_register(
                        'RX_ADDR_P1', rx_addr_p1.to_bytes(5, 'big')
                    )
                else:
                    print("Error: Addreses length must be 5 bytes.")
        if args.rx_addr_p3:
            if args.rx_addr_p3 is True:
                ...
                if len(int(args.rx_addr_p3, 16).to_bytes(1, 'big')) == 5:
                    ...
                    nrf24l01.w_register(
                        'RX_ADDR_P3', rx_addr_p3.to_bytes(1, 'big')
                    )
                else:
        ...
        if args.rx_addr_p4:
            if args.rx_addr_p4 is True:
                ...
                if len(int(args.rx_addr_p4, 16).to_bytes(1, 'big')) == 5:
                    ...
                    nrf24l01.w_register(
                        'RX_ADDR_P4', rx_addr_p4.to_bytes(1, 'big')
                    )
                else:
                    print("Error: Addreses length must be 1 byte.")
        if args.rx_addr_p5:
            if args.rx_addr_p5 is True:
                ...
                if len(int(args.rx_addr_p5, 16).to_bytes(1, 'big')) == 5:
                    ...
                    nrf24l01.w_register(
                        'RX_ADDR_P5', rx_addr_p5.to_bytes(1, 'big')
                    )
                else:
                    print("Error: Addreses length must be 1 byte.")
        if args.tx_addr:
    ...
    # bit is cleared and then then PWR_UP is set.
        ...
        nrf24l01.w_register(
            'CONFIG', new_config_register_value.to_bytes(1, 'big')
        )
        ...
        if args.pipe != None:  # If the user species a pipe
            ...
            if 0 <= args.pipe <= 5:
                pipe_address = nrf24l01.r_register(
                    ('RX_ADDR_P' + str(args.pipe))
                )
                ...
                raise ValueError(
                    "The specified pipe must be in the range [0,5]."
                )
    ...
    ############################################################################
        ...
        if args.detach:
            ...
            while True:
                status_value = int.from_bytes(
                    nrf24l01.r_register('STATUS'), 'big'
                )
                ...
                if rx_dr_value == 1:
                    received_data = nrf24l01.r_rx_payload(
...
# TODO: Change from if statements to functions? This might be better supted to
# switching to click. Changing to functions would make the code easier to
# follow, and more readable I think. Section the functions for each subcommand?
# TODO: Add docstrings?
...
# TODO: Add binary and decimal output to reset?creates
