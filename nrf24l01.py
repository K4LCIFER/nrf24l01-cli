...
REGISTER_MAP = {
    ...
    'TX_ADDR': {'ADDRESS': 0x10, 'NUMBER_OF_BYTES': 5},
    'RX_PW_P0': {
        'ADDRESS': 0x11,
        'NUMBER_OF_BYTES': 1,
...
def r_register(register_name):
    with serial.Serial(PORT, BAUD, timeout=1) as ser:
        ...
        ser.write(
            (REGISTER_MAP[register_name]['NUMBER_OF_BYTES'] + 1).to_bytes(
                1, 'big'
            )
        )
        ...
        uart_response = int.from_bytes(
            ser.read(
                REGISTER_MAP[register_name]['NUMBER_OF_BYTES'] + 1,
            ),
            'big',
        )
        ...
        if REGISTER_MAP[register_name]['NUMBER_OF_BYTES'] == 1:
            ...
            for bit_mnemonic in REGISTER_MAP[register_name]:
                # Ignore the ADDRESS and NUMBER_OF_BYTES keys.
                if (
                    bit_mnemonic != 'ADDRESS'
                    and bit_mnemonic != 'NUMBER_OF_BYTES'
                ):
        ...
        elif REGISTER_MAP[register_name]['NUMBER_OF_BYTES'] > 1:
